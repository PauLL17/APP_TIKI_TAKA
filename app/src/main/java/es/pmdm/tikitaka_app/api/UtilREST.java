...
public class UtilREST {
    ...
    // Clase Request que encapsula la información de la petición HTTP (consulta, url, listener, datos y token JWT).
    ...
    // Acepta el tipo de consulta, una URL, datos opcionales, token JWT y un OnResponseListener.
    // Este método instancia un objeto Request y ejecuta una tarea asíncrona PrvDownloadTask para realizar la solicitud HTTP.
    public static void runQuery(QueryType type, String strUrl, String data, String token, OnResponseListener listener) {
    ...
    private static class PrvDownloadTask extends AsyncTask<Request, Void, Response> {
        ...
        protected Response doInBackground(Request... params) {
            ...
            try {
                ...
                http.setRequestProperty("Accept", "application/json");                          // Decimos qué esperamos recibir
                http.setRequestProperty("Content-Type", "application/json; charset=utf-8");     // Decimos qué estamos enviando

                // Añadimos la cabecera JWT si existe
                if (mRequest.token != null && !mRequest.token.isEmpty()) {
                    http.setRequestProperty("Authorization", "Bearer " + mRequest.token);
                }
